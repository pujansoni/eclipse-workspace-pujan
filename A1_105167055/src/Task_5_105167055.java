...
public class Task_5_105167055 {
	...
	static void getTreeDetails(String treeName) {
		System.out.println(treeName + " details");
	...
	static long insertInTree(Object typeOfSearchTree) {
		...
		for( int i = 1; i <= RECORDS; i++) {
			...
			if(typeOfSearchTree instanceof AVLTree) {
				...
				((AVLTree<Integer>) typeOfSearchTree).insert(randomInt);
				endTime = System.nanoTime();
			} else if(typeOfSearchTree instanceof SplayTree) {
				startTime = System.nanoTime();
				((SplayTree<Integer>) typeOfSearchTree).insert(randomInt);
				endTime = System.nanoTime();
			} else if(typeOfSearchTree instanceof RedBlackBST) {
				startTime = System.nanoTime();
				((RedBlackBST<Integer, Integer>) typeOfSearchTree).put(randomInt, randomInt);
				endTime = System.nanoTime();
			} else {
				startTime = System.nanoTime();
				((BinarySearchTree<Integer>) typeOfSearchTree).insert(randomInt);
				endTime = System.nanoTime();
			}
	...
	static long removeFromTree(Object typeOfSearchTree) {
		...
		for( int i = 1; i <= RECORDS; i++) {
			...
			if(typeOfSearchTree instanceof AVLTree) {
	    		startTime = System.nanoTime();
	    		((AVLTree<Integer>) typeOfSearchTree).remove(randomInt);
	    		endTime = System.nanoTime();
	    	} else if(typeOfSearchTree instanceof SplayTree) {
	    		startTime = System.nanoTime();
	    		((SplayTree<Integer>) typeOfSearchTree).remove(randomInt);
	    		endTime = System.nanoTime();
	    	} else if(typeOfSearchTree instanceof RedBlackBST) {
	    		startTime = System.nanoTime();
	    		((RedBlackBST<Integer, Integer>) typeOfSearchTree).delete(randomInt);
	    		endTime = System.nanoTime();
	    	} else {
	    		startTime = System.nanoTime();
	    		((BinarySearchTree<Integer>) typeOfSearchTree).remove(randomInt);
	    		endTime = System.nanoTime();
	    	}
	    	totalTime += (endTime - startTime);
	    }
